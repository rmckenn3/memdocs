...
>> [!IMPORTANT]
>The following certificate templates are default templates included with AD CS enterprise certification authorities (CAs). These are used by the NDES role configuration wizard to request and enroll certain certificates that are required to allow the server to request certificates on behalf of devices and to encrypt communications between the server and devices. These default templates are version 1 certificate templates which have functional and security limitations; notably, the lack of support for autoenrollment. When the server certificates generated by these templates eventually expire, you will be unable to renew them as they are user certificates enrolled in the computer store.
>
> You cannot instruct the NDES role configuration wizard to use different templates for these certificates. For this reason, Microsoft recommends duplicating these templates, making changes to make them more secure and enabling autoenrollment to make certificate renewals more seamless. IT admins can complete the NDES role configuration wizard normally and then delete the certificates it creates. They can then request and enroll replacement certificates using the newly created templates. These steps are documented later in this article.
...
> |**Compatibility settings** | - Certification Authority: Windows Server 2016
...
> |**Subject Name**|Choose **Build from this Active Directory information**.|
...
3. Using **certlm.msc**, delete the two RA certificates located in the Computer Certificate store under Personal > Certificates. They can be identified by their Certificate Templates: **CEP Encryption** and **Exchange Enrollment Agent (Offline Request)**.
4. Request and enroll new certificates using the templates created in the previous section by right-clicking in a blank space in the personal certificates folder within certlm, selecting *All tasks*, *Request New Certificate* and follow the steps. Make sure to use the certificate templates you duplicated and modified and not the default templates.
