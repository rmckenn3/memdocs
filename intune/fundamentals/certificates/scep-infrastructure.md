...
> [!IMPORTANT]
>The following certificate templates are included by default with all AD CS enterprise certification authorities (CAs). They are used by the NDES role configuration wizard to request and enroll certain certificates that allow the server to request certificates on behalf of devices and to encrypt communications between the server and devices. These default templates are version 1 certificate templates which have functional and security limitations; notably, the lack of support for auto enrollment. When the server certificates generated by these templates eventually expire, you will be unable to renew them as they are user certificates enrolled in the computer store.
>
> You cannot instruct the NDES role configuration wizard to use different templates for these certificates. For this reason, Microsoft recommends duplicating these templates, modifying them to make them more secure and enabling auto enrollment to make certificate renewals more seamless. IT admins can complete the NDES role configuration wizard normally and then delete the certificates it creates. They can then request and enroll replacement certificates using the newly created templates. These steps are documented in the [Duplicate RA certificate templates](#duplicate-ra-certificate-templates) step of this article.
> |Object    |Details    |
> |----------|-----------|
...
[!INCLUDE (#duplicate-ra-certificate-templates)]
