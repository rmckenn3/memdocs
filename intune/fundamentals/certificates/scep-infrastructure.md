...
> [!IMPORTANT]
>The following certificate templates are included by default on all AD CS enterprise certification authorities (CAs). They are used by the NDES role configuration wizard to request and enroll certain certificates that allow the server to request certificates on behalf of devices and to encrypt communications between the server and devices. These default templates are version 1 certificate templates which have functional and security limitations; notably, the lack of support for auto enrollment. See [Certificate templates concepts](/windows-server/identity/ad-cs/certificate-template-concepts) for more details. When the server certificates generated by these templates eventually expire, you will be unable to renew them. Attempting to renew or request a new certificate using these templates will result in an error because the certificates are stored in the computer store despite the templates designating them as user certificates.
>
> You cannot instruct the NDES role configuration wizard to use different certificate templates because the default templates are hard-coded into the wizard. For this reason, Microsoft recommends duplicating these templates, modifying them to make them more secure and enabling auto enrollment to make certificate renewals more seamless. IT admins can complete the NDES role configuration wizard normally and then delete the certificates it creates. They can then request and enroll replacement certificates using the newly created templates. These steps are documented in the [Duplicate RA certificate templates](#duplicate-ra-certificate-templates) step of this article.
> |Object    |Details    |
> |----------|-----------|
> |**CEP Encryption template**         |Used by the device to encrypt communication with NDES.|
> |**Exchange Enrollment Agent (Offline Request) template**         |Used to request certificates on behalf of another subject.|
> |**IPSec (Offline request template)** |Default template to enroll certificates to devices
...
### Duplicate RA certificate templates

> [!IMPORTANT]
>The following certificate templates are included by default on all AD CS enterprise certification authorities (CAs). They are used by the NDES role configuration wizard to request and enroll certain certificates that allow the server to request certificates on behalf of devices and to encrypt communications between the server and devices. These default templates are version 1 certificate templates which have functional and security limitations; notably, the lack of support for auto enrollment. See [Certificate templates concepts](/windows-server/identity/ad-cs/certificate-template-concepts) for more details. When the server certificates generated by these templates eventually expire, you will be unable to renew them. Attempting to renew them or request a new certificate using these templates will result in an error because the templates specify a user certificate but the NDES server enrolls them in the computer store.
>
> You cannot instruct the NDES role configuration wizard to use different templates for these certificates because the default templates are hard-coded into the wizard. For this reason, Microsoft recommends duplicating these templates, modifying them to make them more secure and enabling auto enrollment to make certificate renewals more seamless. IT admins can complete the NDES role configuration wizard normally and then delete the certificates it creates. They can then request and enroll replacement certificates using the newly created templates. These steps are documented below.

1. Duplicate the *Enrollment Agent (Computer)* and *CEP Encryption* templates from the CA. *Enrollment Agent (Computer)* will replace the *Exchange Enrollment Agent (Offline Request)* template.

2. Configure the following settings on the specified tabs of the template:

| Certificate template tab | Action                                                                                                                                                                                                                                                                             |   |
|--------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---|
| Compatibility settings   | **Certification Authority:** Windows Server 2016<br>**Certificate Recipient:** Windows 10/Windows Server 2016                                                                                                                                                                                 |   |
...
| Issuance Requirements    | Because of NDES’ Enroll-on-behalf capability, the NDES service certificates are very powerful. **We therefore recommend enforcing CA certificate manager approval** for enrollment. Please keep in mind that this will interrupt the automatic renewal process of the certificate. |   |
| Extensions               | The default Application Policy is **Certificate Request Agent**. Do not change it.                                                                                                                                                                                                 |   |
| Security                 | Grant **ENROLL** and **AUTOENROLL** permissions to the **NDES Computer account only**.                                                                                                                                                                                             |   |

3. Save the certificate templates.

## Publish certificate templates

1. On the issuing CA, use the Certification Authority snap-in to publish the certificate template. Select the **Certificate Templates** node, select **Action** > **New** > **Certificate Template to Issue**, and then select the certificate templates you created in the previous sections.
...
### Request and install certificates on the server that hosts NDES

Using **certlm.msc**, delete the two RA certificates located in the Computer Certificate store under Personal > Certificates. They can be identified by their Certificate Templates: **CEP Encryption** and **Exchange Enrollment Agent (Offline Request)**.

#### Option A: Auto enrollment
...
- **Registration Authority (RA) certificates**
    ...
    Request and enroll new certificates using the templates created in [Duplicate RA certificate templates](#duplicate-ra-certificate-templates) by right-clicking in a blank space in the personal certificates folder within certlm, selecting *All tasks*, *Request New Certificate* and then follow the steps. Make sure to use the certificate templates you duplicated and modified and not the default templates.

### Bind certificates on the server that hosts NDES
...
## Set group policy for certificate renewal
In order to faciliate auto enrollment [Duplicate RA certificate templates](#duplicate-ra-certificate-templates),
MOVE UP BECAUSE YOU CAN REPLACE THE INSTRUCTIONS TO MANUALLY ENROLL AND JUST TELL THEM TO DO GPUPDATE INSTEAD.
